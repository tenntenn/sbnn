 ...
 *
 * When there is no such comment - nothing visited yet, it was resolved away,
 * or the reader has since scrolled to another file, which makes the old
 * position stale - the tour rejoins at the file being read: the first of its
 * comments going forward, the last going back. That comment is the
 * destination rather than a place to step off from, because the reader is
 * not standing on it yet. A file with no comments of its own falls back to
 * the first or last comment of the whole review.
 */
export function stepToComment(
  ...
  const at = stops.findIndex(
    (s) => s.id === currentId && (activeKey === null || s.key === activeKey),
  )
  if (at >= 0) {
