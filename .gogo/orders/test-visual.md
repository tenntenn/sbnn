...
**トークンが見つからない場合に失敗させない。** 同じ波で web-tokens レーンが
`styles.css` のトークンを増やし・名前を変えている。上に挙げた 11 個のトークンのうち
CSS に存在しないものがあったら、その組は `t.Logf` で「見つからないので飛ばした」と出して
**スキップする。テストは落とさない。** 落とすようにすると、別レーンの正当な変更が
このテストのせいでマージできなくなる。この方針をテスト内のコメントに書く。
