...
# 指示文 O-05 / doc-api — HTTP API の参照文書を作る
...
現在は 22 本ある（`/_/api/…` が 21 本と `/_/events` の SSE が 1 本）。
**数を自分で数えて書くこと。** issue の本文は「20 endpoints」と言っているが、
**コードのほうが正しい。** 食い違いに気づいたら、コードに合わせ、報告に 1 行書く。
