...
//
// This fixes the parse, not the silence. Issue #136 also asks for a per-file
// warning saying the body did not match its header; that lives on
// model.File and is not added here, so a truncated body is still read without
// comment. Whatever the warning ends up looking like, the counts below are
...
// Every patch git format-patch writes ends with the "-- " signature, and one
// of the two lines below it is a bare version number. Reading past a hunk
// header's counts must not mistake any of that for a deletion: it invents a
// change nobody made, and it drops an additions-only file out of the unified
// view because Deletions is no longer zero.
//
// The fixtures are unedited `git format-patch -1 --stdout` output taken from
// this repository. Each one carries its own answer - the diffstat summary git
// wrote above the diff - so the test checks the parse against what git
// counted rather than against a number typed here.
func TestParseFormatPatchOutput(t *testing.T) {
	tests := []struct {
		name  string
		file  string
		files int
	}{
		{name: "signature after a context line", file: "format-patch-context-tail.patch", files: 1},
		{name: "signature after an added file", file: "format-patch-new-file.patch", files: 2},
		{name: "additions only", file: "format-patch-additions-only.patch", files: 2},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			src := string(b)
			wantAdd, wantDel := diffstatSummary(t, src)

			files := diff.Parse(src)
			if len(files) != tt.files {
				t.Fatalf("got %d file(s), want %d:\n%s", len(files), tt.files, describe(files))
			}
			var gotAdd, gotDel int
			for _, f := range files {
				gotAdd += f.Additions
				gotDel += f.Deletions
				for _, h := range f.Hunks {
					for _, l := range h.Lines {
						// "-- " read as a deletion leaves "- " behind.
						if l.Kind == model.LineDelete && strings.TrimRight(l.Content, " ") == "-" {
							t.Errorf("%s: the format-patch signature was read as a deletion", f.NewPath)
						}
					}
				}
				if f.Deletions == 0 && f.ViewMode != model.ViewUnified {
					t.Errorf("%s: ViewMode = %q, want %q for a file with no deletions",
						f.NewPath, f.ViewMode, model.ViewUnified)
				}
			}
			if gotAdd != wantAdd || gotDel != wantDel {
				t.Errorf("got +%d -%d, want +%d -%d (git's own diffstat)", gotAdd, gotDel, wantAdd, wantDel)
			}
		})
	}
}

// diffstatSummary reads the insertion and deletion counts out of the
// "N files changed, X insertions(+), Y deletions(-)" line format-patch puts
// above the diff.
func diffstatSummary(t *testing.T, src string) (add, del int) {
	t.Helper()
	re := regexp.MustCompile(`(?m)^ \d+ files? changed(?:, (\d+) insertions?\(\+\))?(?:, (\d+) deletions?\(-\))?$`)
	m := re.FindStringSubmatch(src)
	if m == nil {
		t.Fatal("fixture has no diffstat summary line")
	}
	for i, n := range []*int{&add, &del} {
		if m[i+1] != "" {
...
// A hunk's surplus lines end where the patch's wrapping begins. Every case
// here is a real tail: the signature git format-patch writes, the "---" that
// closes a commit message, a diffstat row, and a "--- " header whose "+++ "
// never arrived because the patch was cut short.
func TestParseSurplusHunkLinesStopAtThePatchWrapping(t *testing.T) {
	const body = "--- a/a.txt\n+++ b/a.txt\n@@ -1 +1 @@\n-a\n+b\n"
	tests := []struct {
		name string
		tail string
	}{
		{name: "format-patch signature", tail: "-- \n2.51.0\n"},
		{name: "three-dash separator", tail: "---\n"},
		{name: "signature without a space", tail: "--\n"},
		{name: "diffstat rows", tail: " a.txt | 2 +-\n 1 file changed, 1 insertion(+), 1 deletion(-)\n"},
		{name: "truncated file header", tail: "--- a/b.txt\n"},
		{name: "trailing blank line", tail: "\n"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			files := diff.Parse(body + tt.tail)
			if len(files) != 1 {
				t.Fatalf("got %d file(s), want 1:\n%s", len(files), describe(files))
			}
			f := files[0]
			if f.Additions != 1 || f.Deletions != 1 {
				t.Errorf("got +%d -%d, want +1 -1:\n%s", f.Additions, f.Deletions, describe(files))
			}
			if n := len(f.Hunks[0].Lines); n != 2 {
				t.Errorf("hunk has %d line(s), want 2:\n%s", n, describe(files))
			}
		})
	}
}

// An additions-only file stays in the unified view. It only left it because a
// trailing "-- " was counted as a deletion.
func TestParseAddedFileWithFormatPatchSignatureStaysUnified(t *testing.T) {
	files := diff.Parse("--- /dev/null\n+++ b/n.txt\n@@ -0,0 +1 @@\n+x\n-- \n2.51.0\n")
	if len(files) != 1 {
		t.Fatalf("got %d file(s), want 1:\n%s", len(files), describe(files))
	}
	f := files[0]
	if f.Additions != 1 || f.Deletions != 0 {
		t.Errorf("got +%d -%d, want +1 -0", f.Additions, f.Deletions)
	}
	if f.ViewMode != model.ViewUnified {
...
// Surplus lines that sit behind an empty context line are still surplus. The
// empty line is ambiguous on its own - a diff may simply end with one - so it
// is the change line past it that proves the body has not finished.
func TestParseKeepsSurplusHunkLinesAfterAnEmptyContextLine(t *testing.T) {
	files := diff.Parse("--- a/a.txt\n+++ b/a.txt\n@@ -1,1 +1,1 @@\n-a\n+b\n\n-c\n+d\n")
	if len(files) != 1 || len(files[0].Hunks) != 1 {
		t.Fatalf("want 1 file with 1 hunk, got:\n%s", describe(files))
	}
	...
	want := "delete:a add:b context: delete:c add:d"
